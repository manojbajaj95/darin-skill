...
Darin is meant to fix that. It's a skill you run inside Cursor, Claude Code, Codex, Gemini CLI, or pretty much any AI tool that supports Agent Skills, not a separate app you have to remember to open. Feed it interviews, support tickets, or usage numbers, and it keeps track of what's an observation, what's a guess, and what's an actual decision. When you sit down to plan a feature or rank the backlog, that history is already there with the reasoning attached.
...
Then in your harness: **`/darin init`** → **`/darin ingest`** → **`/darin plan`**
...
The whole thing is a simple loop: **capture what you learn, then plan what you build.**
...
3. **When you need to decide what to build — `/darin plan`.** Give it a problem or a goal. It pulls the relevant things you've captured and hands back a scoped brief: what to build now, what's next, what to skip, and how you'll know it worked.

That's the core. Two extras are there when you need them:
